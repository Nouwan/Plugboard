    ...
    where T : IEquatable<T>
{
    ...
    {
        return obj is EquatableArray<T> objAsArray && Equals(objAsArray);
    }
    ...
    public override int GetHashCode()
    {
        ...
        unchecked
        {
            return _array.Aggregate(17, (current, item) => current * 31 + (item?.GetHashCode() ?? 0));
        }
    }
    ...
    private ImmutableArray<T> AsImmutableArray()
    {
        // default(EquatableArray<T>) holds a default ImmutableArray; expose it as empty so enumeration never throws.
        return _array.IsDefault ? [] : _array;
    }
    ...
    private static EquatableArray<T> FromImmutableArray(ImmutableArray<T> array) => new(array);
